...
from PySide6.QtGui import QAction, QPainterPath, QColor, QPen, QBrush, QPixmap, QKeySequence
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.next_annotation_size: int = 15
    ...
    def setup_ui(self):
        ...
        self.property_editor = PropertyEditor(); right_layout.addWidget(self.property_editor)
    ...
    def setup_compact_ocr_panel(self, parent_layout):
        ...
        row2_layout = QHBoxLayout(); self.enhance_contrast_cb = QCheckBox("增强对比度"); self.enhance_contrast_cb.setChecked(True); row2_layout.addWidget(self.enhance_contrast_cb); self.denoise_cb = QCheckBox("降噪"); self.denoise_cb.setChecked(True); row2_layout.addWidget(self.denoise_cb); self.gpu_checkbox = QCheckBox("GPU"); self.gpu_checkbox.setChecked(HAS_GPU_SUPPORT); self.gpu_checkbox.setEnabled(HAS_GPU_SUPPORT); row2_layout.addWidget(self.gpu_checkbox); row2_layout.addStretch(); ocr_layout.addLayout(row2_layout)
    ...
    def setup_toolbar(self):
        ...
        self.area_select_action = QAction("区域标注", self); self.area_select_action.setCheckable(True); self.area_select_action.setShortcut("Q"); self.area_select_action.setStatusTip("激活后，在图纸上拖拽鼠标以创建一个标注"); self.area_select_action.toggled.connect(self.toggle_area_selection); toolbar.addAction(self.area_select_action)
        ...
        toolbar.addSeparator()
        toolbar.addWidget(QLabel("大小:")); self.size_slider = QSlider(Qt.Horizontal); self.size_slider.setRange(8, 30); self.size_slider.setValue(self.next_annotation_size); self.size_slider.setFixedWidth(80); self.size_label = QLabel(str(self.next_annotation_size)); self.size_label.setFixedWidth(25); toolbar.addWidget(self.size_slider); toolbar.addWidget(self.size_label)
        toolbar.addSeparator()
    ...
    def create_annotations_from_ocr(self):
        ...
        created_count = 0; confidence_threshold = self.confidence_slider.value() / 100.0
        for result in self.ocr_results:
    ...
    def create_annotation_from_ocr_result(self, ocr_result: dict):
        ...
        self._create_new_annotation(
            anchor_point=QPointF(ocr_result['center_x'], ocr_result['center_y']),
    ...
    def on_annotation_selected(self, annotation: BubbleAnnotationItem):
        ...
        self.size_slider.blockSignals(True); self.size_slider.setValue(annotation.radius); self.size_label.setText(str(annotation.radius)); self.size_slider.blockSignals(False)
    ...
    def clear_annotations(self):
        ...
        self.current_annotation = None
        self.annotation_counter = 0
        self.status_bar.showMessage("所有标注已清除", 2000)
